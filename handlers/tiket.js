    ...
    return;
        ...
        if (interaction.isButton() && interaction.customId === 'done_tiket') {
            ...
            try {
                ...
                messages.reverse().forEach(m => {
    // 1. Catat waktu dan penulis
    logContent += `[${m.createdAt.toLocaleString()}] ${m.author.tag}: `;
    
    // 2. Jika ada konten teks, tambahkan ke log
    if (m.content) {
        logContent += `${m.content}`;
    }
    logContent += `\n`;

    // 3. Jika ada embed, baca isinya (terutama field-nya)
    if (m.embeds && m.embeds.length > 0) {
        m.embeds.forEach(embed => {
            if (embed.fields) {
                embed.fields.forEach(field => {
